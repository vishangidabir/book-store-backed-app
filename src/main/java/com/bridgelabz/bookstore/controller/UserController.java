...
public class UserController {
     ...
     */
    @GetMapping("/get/{id}")
    public ResponseEntity<ResposeDTO> getById(@RequestHeader String token) {
        ResposeDTO dto = new ResposeDTO("Record retrieved successfully !", userService.getUserById(token));
        return new ResponseEntity(dto, HttpStatus.OK);
    }
...
//    }
    ...
    @PostMapping("/login")
    public ResponseEntity<ResposeDTO> login(@RequestBody LoginDTO loginDTO) {
        ResposeDTO responseDTO = new ResposeDTO("LOGIN SUCCESSFUL", userService.login(loginDTO));
        return new ResponseEntity<>(responseDTO, HttpStatus.OK);
    }
